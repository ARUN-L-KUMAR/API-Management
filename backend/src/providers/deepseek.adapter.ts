...
export class DeepSeekAdapter implements ProviderAdapter {
  ...
  async fetchModels(apiKey: string): Promise<DiscoveredModel[]> {
    try {
      ...
      return list.map((model: any) => {
        ...
        return {
          id,
          displayName: id.split('/').pop().replace(/-/g, ' ').toUpperCase(),
          capabilities: {
            text: true,
  ...
  private handleError(error: any): ValidationResult {
    if (error.response) {
      ...
      if (status === 401) {
        return { status: 'Unauthorized', errorMessage: 'Invalid DeepSeek API Key.' };
      }
      if (status === 402) {
        return { status: 'Quota Exceeded', errorMessage: 'Insufficient DeepSeek balance.' };
      }
      if (status === 429) {
        return { status: 'Rate Limited', errorMessage: 'DeepSeek rate limit exceeded.' };
      }
