...
export class GroqAdapter implements ProviderAdapter {
  ...
  private handleError(error: any): ValidationResult {
    if (error.response) {
      ...
      if (status === 401) {
        return { status: 'Unauthorized', errorMessage: 'Invalid Groq API Key.' };
      }
      if (status === 403) {
        return { status: 'Unauthorized', errorMessage: 'Groq authorization failed.' };
      }
      if (status === 429) {
        return { status: 'Rate Limited', errorMessage: 'Groq rate limit exceeded.' };
      }
